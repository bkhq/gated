...
use gated_common::{LogFormat, GatedConfig};
...
pub async fn init_logging(config: Option<&GatedConfig>, cli: &Cli) -> Result<()> {
    if std::env::var("RUST_LOG").is_err() {
        match cli.debug {
            0 => std::env::set_var("RUST_LOG", "gated=info"),
            1 => std::env::set_var("RUST_LOG", "gated=debug"),
            2 => std::env::set_var("RUST_LOG", "gated=debug,russh=debug"),
