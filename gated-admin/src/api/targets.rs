...
use uuid::Uuid;
use gated_common::{
    Role as RoleConfig, Target as TargetConfig, TargetOptions, TargetSSHOptions, GatedError,
};
...
impl ListApi {
    ...
    async fn api_get_all_targets(
        ...
        let targets = targets.all(&*db).await.map_err(GatedError::from)?;
...
impl DetailApi {
    ...
    async fn api_update_target(
        ...
        model.options =
            Set(serde_json::to_value(body.options.clone()).map_err(GatedError::from)?);
